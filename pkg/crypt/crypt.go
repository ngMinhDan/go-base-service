/*
Package crypt: Provides methods for encrypting and decrypting data using RSA.
*/
...
func init() {
	...
	KeyRSACfg.BytePrivate, err = ioutil.ReadFile(config.Config.GetString("CRYPT_PRIVATE_KEY_FILE"))
	if err != nil {
	...
	KeyRSACfg.BytePublic, err = ioutil.ReadFile(config.Config.GetString("CRYPT_PUBLIC_KEY_FILE"))
	if err != nil {
		log.Println(log.LogLevelFatal, "init-crypt", err.Error())
	}
